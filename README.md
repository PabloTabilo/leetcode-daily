...
| Problem                                | My personal difficulty | Techniques and comments                              | Solution| Time Complexity | Space Complexity |
| -------------------------------------- | ---------------------- | ---------------------------------------------------- | ---- | ---- | ---- |
| 368. Largest Divisible Subset | medium-hard | `dp` or `LIS` | [cpp](medium/368_LargestDivisibleSubset.cpp) | $$O(n^2)$$ the solution use previous path for get the best answer, but must iterate the inmediate neighbours  | $$O(n^2)$$ for build the graph, on worst case each node has a reference to each other|
...
| 1922. Count Good Numbers | medium | `modular Exponentiation` + `Combinatorics` | [Python3](medium/1922_CountGoodNumbers.py) | $$O(log(expo))$$ where `expo` is the exponential, or the number of times where `base` is multiplied by itself  | $$O(log(expo))$$ the size of the recursive call|
| 2537. Count the Number of Good Subarrays | medium-hard | `Sliding Window` + `Combinatorics` | [Python3](medium/2537_CountTheNumberOfGoodSubarrays.py) | $$O(n)$$  | $$O(k)$$ where $k$ is number of distinct elements |